#!/usr/bin/env python3
"""
Convert Stitched Planes to Pyramidal OME-ZARR (Hardware Overdrive)
====================================================================
Squeezing performance for 32-core Threadripper + NVMe Scratch.
"""

import sys
import time
import warnings
from pathlib import Path
import numpy as np
import zarr
import numcodecs
from tifffile import imread
import dask.array as da
from dask import delayed
from dask.diagnostics import ProgressBar
import dask

# Optimization: Use more threads for the I/O and compression tasks
dask.config.set(scheduler='threads', num_workers=48)

warnings.filterwarnings("ignore", category=FutureWarning)
warnings.filterwarnings("ignore", message=".*The 'zarr' library.*")

def main():
    start_time = time.time()

    # ==================== CONFIGURATION ====================
    INPUT_DIR = "18624_d3"
    OUTPUT_ZARR = "18624_d3.zarr"
    
    # OVERDRIVE CHUNKS: Larger chunks utilize NVMe sequential speeds better.
    # Increasing Y,X to 512 and Z to 64 for high-bandwidth writing.
    CHUNK_SIZE = (1, 64, 512, 512) 
    LEVELS = 4  
    VOXEL_SIZE = [2.0, 1.0, 1.0] 
    # =======================================================
    
    script_dir = Path(__file__).resolve().parent
    input_dir = script_dir / INPUT_DIR
    output_path = script_dir / OUTPUT_ZARR
    
    if not input_dir.exists():
        print(f"ERROR: {input_dir} not found")
        return 1
    
    # 1. ORGANIZE FILES
    tif_files = sorted(input_dir.glob("*.tif"))
    if not tif_files:
        print(f"ERROR: No TIFF files in {input_dir}")
        return 1
    
    channels = {}
    for tif_path in tif_files:
        parts = tif_path.stem.split('_')
        c, z = int(parts[0][1:]), int(parts[1][1:])
        if c not in channels: channels[c] = {}
        channels[c][z] = tif_path
    
    n_c, n_z = len(channels), max(len(z_slices) for z_slices in channels.values())
    first_img = imread(list(channels[0].values())[0])
    n_y, n_x, dtype = *first_img.shape, first_img.dtype
    
    print(f"Dataset: {n_c}C, {n_z}Z, {n_y}Y, {n_x}X")
    print(f"Applying Overdrive: Chunk size {CHUNK_SIZE} on Samsung 9100 PRO ")

    # 2. BUILD LAZY DASK STACK
    @delayed
    def load_plane(path):
        return imread(path)
    
    channel_stacks = []
    for c in sorted(channels.keys()):
        z_slices = [channels[c].get(z) for z in range(n_z)]
        lazy_planes = [
            da.from_delayed(load_plane(p), shape=(n_y, n_x), dtype=dtype) if p 
            else da.zeros((n_y, n_x), dtype=dtype) 
            for p in z_slices
        ]
        channel_stacks.append(da.stack(lazy_planes, axis=0))
    
    volume = da.stack(channel_stacks, axis=0).rechunk(CHUNK_SIZE)

    # 3. WRITE PYRAMID LEVELS
    output_str = str(output_path)
    root = zarr.open_group(output_str, mode='w')
    multiscales_metadata = []
    
    # Using ZSTD with Shuffle is great for 16-bit TIFFs
    blosc_compressor = numcodecs.Blosc(cname='zstd', clevel=3, shuffle=numcodecs.Blosc.BITSHUFFLE)
    
    current_vol = volume
    for level in range(LEVELS):
        level_path = str(level)
        print(f"\nProcessing Level {level} {current_vol.shape}...")
        
        with ProgressBar():
            da.to_zarr(
                current_vol, 
                output_str, 
                component=level_path, 
                overwrite=True,
                compressor=blosc_compressor
            )
        
        s = 2**level
        multiscales_metadata.append({
            "path": level_path,
            "coordinateTransformations": [{"type": "scale", "scale": [1.0, VOXEL_SIZE[0]*s, VOXEL_SIZE[1]*s, VOXEL_SIZE[2]*s]}]
        })
        
        if level < LEVELS - 1:
            current_vol = da.coarsen(np.mean, current_vol, {1: 2, 2: 2, 3: 2}, trim_excess=True).astype(dtype)

    # 4. METADATA
    root.attrs['multiscales'] = [{'version': '0.4', 'datasets': multiscales_metadata}]
    
    total_seconds = time.time() - start_time
    print(f"\n{'='*40}\nOVERDRIVE COMPLETE\nTotal Time: {int(total_seconds//60)}m {total_seconds%60:.2f}s\n{'='*40}")

if __name__ == "__main__":
    main()
